...
void files_in_group_callback(void *context, uint32_t index) {
    ...
    strncpy(app_state.selected_path, app_state.db.records[record_index].path,
    ...
    dialog_ex_set_text(app_state.confirm_dialog, app_state.selected_path, 64, 30, AlignCenter,
                       AlignTop);
...
void render_files_in_group(size_t group_index) {
    ...
    for (size_t i = 0; i < group->count; i++) {
        submenu_add_item(app_state.files_in_group_submenu,
                         app_state.db.records[group->start_index + i].path, i,
                         files_in_group_callback, NULL);
    }
}
...
void render_groups_submenu() {
    ...
    for (size_t i = 0; i < app_state.db.num_groups; i++) {
        char group_name[64];
        snprintf(group_name, sizeof(group_name), "Dup Group %zu (%zu)", i + 1,
    ...
    if (storage_dir_open(dir, "/ext/subghz")) {
        ...
        while (storage_dir_read(dir, &file_info, filename, sizeof(filename)) &&
            ...
            if (!file_info_is_dir(&file_info)) {
                ...
                strncpy(app_state.db.records[app_state.db.count].path, filename,
...
int32_t sub_dup_finder_app(void *p) {
    ...
    submenu_add_item(app_state.main_submenu, "Credits", SubDupFinderSubmenuIndexCredits,
                     main_submenu_callback, NULL);
    ...
    view_dispatcher_add_view(app_state.view_dispatcher, SubDupFinderViewSubmenu,
                             submenu_get_view(app_state.main_submenu));
    view_dispatcher_add_view(app_state.view_dispatcher, SubDupFinderViewGroups,
                             submenu_get_view(app_state.groups_submenu));
    view_dispatcher_add_view(app_state.view_dispatcher, SubDupFinderViewFiles,
                             submenu_get_view(app_state.files_in_group_submenu));
    view_dispatcher_add_view(app_state.view_dispatcher, SubDupFinderViewConfirm,
                             dialog_ex_get_view(app_state.confirm_dialog));
    view_dispatcher_add_view(app_state.view_dispatcher, SubDupFinderViewCredits,
                             widget_get_view(app_state.credits_widget));
    view_dispatcher_add_view(app_state.view_dispatcher, SubDupFinderViewPopup,
                             popup_get_view(app_state.popup));
